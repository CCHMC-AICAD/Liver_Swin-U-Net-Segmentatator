...
def main():
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            # val_outputs_copy = np.copy(val_outputs)
            val_outputs_copy= np.squeeze(val_outputs, axis=0)
            # print("val_outputs:", val_outputs_copy.shape)
            
            img = nib.Nifti1Image(val_outputs_copy, np.eye(4))
            nib.save(img, os.path.join('results_test_lr-5/', img_name))
            # print(os.path.join('results_test_lr-5/', img_name))
            val_labels = val_labels.cpu().numpy()[:, 0, :, :, :]
            dice_list_sub = []
            for i in range(1, 3):
            # for i in range(1, 14):
                organ_Dice = dice(val_outputs[0] == i, val_labels[0] == i)
                print("1 is liver, 2 is spleen")
                print(organ_Dice)
            ...
            print("Mean Organ Dice per subject: {}".format(mean_dice))
